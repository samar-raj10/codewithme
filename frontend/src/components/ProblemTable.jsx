...
export function ProblemTable({
  ...
  return (
    <div className="p-5 rounded-2xl bg-white dark:bg-leetcode-dark-card border border-gray-200 dark:border-leetcode-dark-border shadow-sm space-y-4">
      ...
      {/* Filter Tabs */}
      <div className="flex flex-col sm:flex-row sm:items-center gap-2 pb-1 border-b border-gray-100 dark:border-zinc-800 text-xs">
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg font-medium whitespace-nowrap transition-all ${
                filterStatus === tab.key
                  ? "border border-leetcode-orange text-leetcode-orange font-semibold bg-leetcode-orange/10"
        ...
        <div className="relative sm:ml-auto w-full sm:max-w-xs">
          ...
          <input
            type="text"
            aria-label="Search problems by number or title"
            placeholder="Search by # or title..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full pl-9 pr-3 py-1.5 text-xs rounded-lg border border-gray-200 dark:border-zinc-700 bg-gray-50 dark:bg-zinc-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-leetcode-orange/50 transition-all"
          />
        </div>
      </div>
      ...
      {problems.length === 0 ? (
        <div className="py-12 text-center text-gray-500 dark:text-gray-400">
          ...
          <p className="text-xs mt-1">
            Try clearing your filters or log a new problem attempt above.
          </p>
        </div>
      ) : (
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-zinc-800/60">
              {problems.map((problem) => {
                const statusConfig =
                  STATUS_BADGE_CONFIG[problem.status] ||
                  STATUS_BADGE_CONFIG.pending;
                const stageBadge =
                ...
                const isDueOrOverdue =
                  problem.status === "due" || problem.status === "overdue";
                  ...
                  >
                    ...
                    <td className="py-3.5 px-3 max-w-xs">
                      {!problem.notes || !problem.notes.trim() ? (
                        <span className="text-gray-400 dark:text-gray-600 text-[11px] italic">
                          No notes
                        </span>
                      ) : !isNoteRevealed ? (
                        <button
                          onClick={() => toggleNoteHint(problem._id)}
                          className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-lg bg-amber-500/10 hover:bg-amber-500/20 text-amber-700 dark:text-amber-400 border border-amber-500/20 text-[11px] font-semibold transition-all shadow-sm group-hover:border-amber-500/40"
                    ...
                    <td className="py-3.5 px-3 whitespace-nowrap">
                      <span
                        className={`px-2 py-0.5 rounded border text-[11px] font-medium ${stageBadge}`}
                      >
                        {stageLabel}
                      </span>
                    </td>
                    ...
                    <td className="py-3.5 px-3 whitespace-nowrap">
                      <span
                        className={`px-2.5 py-0.5 rounded-full border text-[11px] ${statusConfig.classes}`}
                      >
                        {statusConfig.label}
                      </span>
                    </td>
